...
class RemoveRetailerPage extends StatefulWidget {
  @override
  _RemoveRetailerPageState createState() => _RemoveRetailerPageState();
}

class _RemoveRetailerPageState extends State<RemoveRetailerPage> {
  ...
  Future<List<Technician>> _fetchTechniciansData() async {
    final response = await Supabase.instance.client.from('retailer').select();
  ...
  Future<void> _removeTechnician(int technicianId) async {
    final response = await Supabase.instance.client.from('retailer').delete().eq('id', technicianId);
